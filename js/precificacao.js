...
function atualizarResultado(faixaForcada) {
  ...
  if (selectedLojasIds.length > 1 && multiCards) {
    ...
    canalResultados.forEach(c => {
      ...
      col.innerHTML = `
        <div class="card p-2 h-100 sa-mini-channel-card d-flex flex-column justify-content-between ${isAtivoAtual ? 'active-channel' : ''}" 
             style="border-left: 3.5px solid ${borderBrand} !important; cursor: pointer;"
             onclick="selecionarCanalUnico('${c.id}')">
          
          <!-- Linha 1: Ícone + Nome da Loja + Espaço + Estrela Maior Margem (com Tooltip no Hover) | Preço Sugerido com Botão de Copiar -->
          <div class="d-flex justify-content-between align-items-center mb-1">
            <div class="d-flex align-items-center text-truncate pe-1">
              <i class="bi ${icon} me-1.5" style="color: ${borderBrand}; font-size: 0.85rem;"></i>
              <strong class="text-dark me-2 text-truncate" style="font-size: 0.80rem;">${escapeHtml(c.nomeLoja)}</strong>
              ${isBest ? `<span class="badge bg-warning bg-opacity-25 text-warning border border-warning border-opacity-40 rounded-circle p-0 ms-1 d-inline-flex align-items-center justify-content-center" style="width: 18px; height: 18px; flex-shrink: 0;" title="Maior margem de lucro (${c.margem.toFixed(1)}%)"><i class="bi bi-star-fill text-warning" style="font-size: 0.58rem;"></i></span>` : ''}
            </div>
            <div class="d-flex align-items-center gap-1 text-nowrap ms-auto">
              <span class="fw-bold text-dark" style="font-size: 0.92rem;">${formatCurrency(c.precoVenda)}</span>
              <button type="button" class="btn btn-sm btn-link p-0 text-muted sa-copy-btn" title="Copiar preço deste canal" onclick="copiarPrecoCanal(${c.precoVenda}, this, event)" style="font-size: 0.78rem; line-height: 1;" aria-label="Copiar preço">
                <i class="bi bi-clipboard"></i>
              </button>
            </div>
          </div>

          <!-- Linha 2: Lucro & Margem na esquerda + Taxas na direita (sem cortar números) -->
          <div class="d-flex justify-content-between align-items-center pt-1 border-top border-dark border-opacity-10 text-nowrap" style="font-size: 0.69rem;">
            <div>
              <span class="text-muted">Lucro:</span>
              <strong class="${c.lucro < 0 ? 'text-danger' : 'text-success'} ms-0.5">${formatCurrency(c.lucro)}</strong>
              <span class="text-muted ms-0.5" style="font-size: 0.62rem;">(${c.margem.toFixed(1)}%)</span>
            </div>
            <div class="text-muted ms-2">
              Taxas: <strong class="text-dark ms-0.5">${formatCurrency((c.valorTaxaPercentual || 0) + (c.taxaFixa || 0))}</strong>
            </div>
          </div>
        </div>
      `;
